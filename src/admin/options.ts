    ...
    actions: {
      show: {
        ...
        handler: async (request, response, context) => {
          ...
          if (!id) return { record: context.record?.toJSON() }
    ...
    actions: {
      ...
      show: {
        ...
        handler: async (req, res, ctx) => {
          ...
          if (!id) return { record: ctx.record?.toJSON() }
    ...
    actions: {
      show: {
        ...
        handler: async (request, response, context) => {
          ...
          if (context.record && uni) context.record.params._counts = uni._count;
          return { record: context.record?.toJSON() };
        },
      },
      ...
      bulkDelete: {
        ...
        handler: async (request, response, context) => {
          const ids = (context.records || []).map((r: any) => r.id()).filter(Boolean);
          if (!ids.length) return { records: context.records?.map((r: any) => r.toJSON()) || [], notice: { message: 'No records selected', type: 'info' } };
    ...
    actions: {
      ...
      show: {
        ...
        handler: async (request, response, context) => {
          ...
          if (context.record && faculty) { context.record.params.university = faculty.university; context.record.params._counts = faculty._count; }
          return { record: context.record?.toJSON() };
        }
      },
    }
  ...
  options: {
    ...
    actions: {
      ...
      show: {
        ...
        handler: async (request, response, context) => {
          const id = context.record?.id(); if (!id) return { record: context.record?.toJSON() };
          ...
          if (context.record && dept) { context.record.params.faculty = dept.faculty; context.record.params.university = dept.university; context.record.params._counts = dept._count; }
          return { record: context.record?.toJSON() };
        }
      },

      edit: {
        // component: Components.DepartmentForm,
        handler: async (request, response, context) => {
          if (request.method === 'get') return { record: context.record?.toJSON() };
          const payload = request.payload || {};
          const id = context.record?.id();
          if (id) {
            const faculty = await prisma.faculty.findUnique({ where: { id: payload.faculty_id } });
            if (!faculty) return { record: context.record?.toJSON(), notice: { message: 'Selected faculty not found', type: 'error' } };
            const duplicate = await prisma.department.findFirst({ where: { name: payload.name, faculty_id: payload.faculty_id, id: { not: id } } });
            if (duplicate) return { record: context.record?.toJSON(), notice: { message: 'Duplicate department name in this faculty', type: 'error' } };
          ...
          if (context.record && updated) context.record.params.facultyName = updated.faculty?.name;
    ...
    actions: {
      list: {
        after: async (response, request, context) => {
          ...
          const map = Object.fromEntries(
            counts.map(c => [c.id, c._count.levels])
          )
      ...
      show: {
        ...
        handler: async (request, response, context) => {
          ...
          if (context.record && fld) { context.record.params.department = fld.department; context.record.params._counts = fld._count;; context.record.params.modules = fld.modules; }
          return { record: context.record?.toJSON() };
        }
      },
    }
...
const buildUserRecord = async (resource, userId: string) => {
...
const options: AdminJSOptions = {
  ...
  branding: {
    ...
    theme: {
      colors: {
        primary100: '#4F46E5', // UniSphere blue
        primary80: '#6366F1',
        accent: '#22C55E',
        hoverBg: '#EEF2FF',
      },
  ...
  dashboard: {
    handler: async () => {
      ...
      const [courses, postsForLikes, postsForComments, teachers2, faculties] =
        await Promise.all([
          prisma.course.findMany({
      ...
      const postByIdForComments = new Map(
        postsForComments.map((post) => [post.id, post]),
      )
        ...
        actions: {
          ...
          confirm: {
            ...
            handler: async (request, response, context) => {
              ...
              // 🔥 Rebuild AdminJS record properly
          ...
          edit: {
            ...
            handler: async (request, response, context) => {
              ...
              // ✅ IMPORTANT: mutate AdminJS record first
