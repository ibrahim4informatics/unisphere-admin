...
export const createHierarchyRoutes = (prisma: PrismaClient) => {
  ...
  router.get('/structure/:universityId', async (req, res) => {
    ...
    const structure = {
      ...
      children: (uni.fculties || []).map((f:any)=>({ id: f.id, name: f.name, type:'faculty', count: f.departments?.length||0, children: (f.departments||[]).map((d:any)=>({ id:d.id, name:d.name, type:'department', count:d.fields?.length||0, children: (d.fields||[]).map((fd:any)=>({ id:fd.id, name:fd.name, type:'field', count: fd.levels?.length||0, children: (fd.levels||[]).map((lv:any)=>({ id:lv.id, name:lv.name, type:'level', count:0, children:[] })) })) })) }))
    };
    res.json(structure);
  });
